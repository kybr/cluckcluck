  ...
  void onAnimate(double dt) override {
    ...
    for (int i = 0; i < agent.size(); i++) {
      ...
      for (int j = 0; j < agent.size(); j++) {
        ...
        if (distance < neighbor_distance) {
          count++;
          sum += them.quat(); // accumulate neighbor positions
        }
      }
      sum += me.quat();
      if (count > 1) {
        Quatd average = sum / (count + 1);
        me.quat(me.quat().slerp(average, 0.5));
      }
    }
    ...
    // Turn a little to match the average heading. The agents may wander off into the distance, in different directions, 
    // so implement wrap around.
